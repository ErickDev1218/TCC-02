...
ROOT_OUTPUT_DIR="results/brkga-cpp"

#Description of the best-so-far configuration:
#   .ID. population_factor  pe  pm rhoe K MAXT X_INTVL X_NUMBER MAX_GENS MAX_STAGT trials .PARENT.
#.30   30                7 0.3 0.2  0.9 3    3     100        2      700       300      1        1
...
for INPUT_DIR in "${INPUT_DIRS[@]}"; do
    ...
    for entry in "${sorted_files[@]}"; do
        ...
        "$EXECUTABLE" "$graph_file" \
            ...
            --rhoe "$RHOE" \
            --K "$NUMBER_INDEPENDENT_POPULATIONS" \
            --MAXT "$NUMBER_OF_THREADS" \
            --X_INTVL "$INTERVAL_EXCHANGE_BEST" \
            --X_NUMBER "$NUMBER_OF_EXCHANGED_INDIVIDUALS" \
            --MAX_GENS "$MAX_GENS" \
            --MAX_STAGT "$MAX_STAGNANT" \
            --trials "$TRIALS" \
            --output "$output_file"
