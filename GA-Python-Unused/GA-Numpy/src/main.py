...
def main():
    ...
    g = Graph(Nodes, Edges)
    ga = GA(population_size=len(Nodes), mutation_rate=0.5, eletism_size=int(len(Nodes) * 0.2), crossover_rate=0.7, generations=1, graph=g, config=["roulette", "one_point", "shift", "partition"])
    ga.ga_flow()
